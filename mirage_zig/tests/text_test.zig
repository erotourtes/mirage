...
test "search markers rebuild after local and remote mutations" {
    ...
    try mirage.debug.ensureSearchMarkers(a.text());
    try std.testing.expect(mirage.debug.searchMarkersValid(a.text()));
    try std.testing.expect(mirage.debug.searchMarkerCount(a.text()) > 1);
    ...
    defer std.testing.allocator.free(update);
    try b.text().applyUpdate(update);
    try std.testing.expect(!mirage.debug.searchMarkersValid(b.text()));
    try mirage.debug.ensureSearchMarkers(b.text());
    try std.testing.expect(mirage.debug.searchMarkersValid(b.text()));

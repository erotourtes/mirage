...
<main class="min-h-svh bg-zinc-950 text-zinc-100">
	<section class="mx-auto flex min-h-svh w-full max-w-6xl flex-col px-5 py-6 sm:px-8 lg:px-10">
		<header class="flex items-center justify-between border-b border-white/10 pb-5">
			<a href="/" class="text-sm font-semibold uppercase tracking-[0.18em] text-cyan-300">Mirage</a>
			<nav class="flex gap-1 text-sm text-zinc-300" aria-label="Primary navigation">
				<a class="rounded-md px-3 py-2 transition hover:bg-white/10 hover:text-white" href="/">Demo</a>
			</nav>
		</header>

		<div class="grid flex-1 items-center gap-10 py-10 lg:grid-cols-[1.05fr_0.95fr] lg:py-14">
			<div class="max-w-2xl">
				<p class="mb-4 text-sm font-medium uppercase tracking-[0.2em] text-cyan-300">
				...
				<p class="mt-6 max-w-xl text-base leading-7 text-zinc-300 sm:text-lg">
					The first page loads the Zig-built WASM module, creates a Mirage document, writes text,
					and reads encoded sync data through a typed wrapper.
				</p>
			</div>
			...
			>
				...
				<div class="space-y-5 p-5">
					{#if loadState === 'error'}
						<p class="rounded-md border border-red-400/40 bg-red-950/40 p-4 text-sm text-red-100">
							{errorMessage}
						</p>
					{:else}
						<div>
							<p class="mb-2 text-xs font-medium uppercase tracking-[0.16em] text-zinc-500">
								Document text
							</p>
							<p class="min-h-24 rounded-md bg-zinc-950 p-4 font-mono text-sm leading-6 text-zinc-100">
						...
						<div class="grid gap-3 sm:grid-cols-3">
							...
							<div class="rounded-md bg-zinc-950 p-4">
								...
								<p class="mt-2 text-2xl font-semibold text-white">{stateVectorBytes} B</p>
							</div>
							<div class="rounded-md bg-zinc-950 p-4">
								<p class="text-xs text-zinc-500">Update</p>
								<p class="mt-2 text-2xl font-semibold text-white">{updateBytes} B</p>
							</div>
						</div>
					{/if}
				</div>
